...
class Graph {
  ...
  constructor(inputPath) {
    ...
    [this.n, this.m, this.degreeArray, this.degreeSum, this.struct] =
      this.readInput(inputPath);
    [this.maxDegree, this.minDegree, this.averageDegree, this.medianDegree] =
      this.getDegreeInfo();
  }

  writeOutput(textList) {
    // Função que escreve no arquivo output.txt
  ...
  getDegreeInfo() {
    ...
    if (this.n % 2 === 0) {
      // Se o número de elementos é par
  ...
  diameter() {
    ...
    for (let v = 1; v <= this.n; v++) {
      // É necessário executar a bfs para cada vértice
      ...
      if (markupVector[i] === Infinity) {
        ...
        for (let k = 0; k < layers.length; k++) {
          if (layers[k] !== Infinity) {
            // Se o vértice estiver marcado da bfs
            size++;
